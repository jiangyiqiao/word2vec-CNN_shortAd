...
tf.flags.DEFINE_string("positive_data_file", "./data/not_ad_4000.txt", "Data source for the positive data.")
tf.flags.DEFINE_string("negative_data_file", "./data/ad_5000.txt", "Data source for the negative data.")
...
timestamp = str(datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))+str(int(time.time())) 
...
with tf.Graph().as_default():
    session_conf = tf.ConfigProto(
        allow_soft_placement = FLAGS.allow_soft_placement,    #打印日志
